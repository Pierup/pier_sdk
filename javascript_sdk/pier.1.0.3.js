( function() {
	this.Pier = function() {
	  ...
	  //CONSTANCE
		// API call
		var URL_PREFIX =  'http://pierup.asuscomm.com:8686';
		var URL_PREFIX2 = 'http://pierup.asuscomm.com:8686';
		var URL = {
		...
		INFOMATION_NOT_COMPLETED = 'Please complete all information in correct way!',
		SEND = 'Get Code',
  ...
  //Pier Checkout
		...
		checkoutNextBtn.setAttribute( 'type', 'button' );
		checkoutNextBtn.setAttribute( 'form', 'popup' );
		...
		var checkoutWrapLogin = document.createElement( 'form' );
		checkoutWrapLogin.setAttribute( 'id', 'popup' );
   ...
   //Checkout
			...
			var loginPromise = templateApiCall( loginUrl, message, function( httpObj ){
				...
				if( httpObj.status == 200 ){
					if( msg.code == 200 ){
						...
						status_bit = msg.result.status_bit;
						if( status_bit == 0 ){
							checkoutErrorMsg( error.invalidAccount );
							return;
						}
		...
		payButton.onclick = function() {
			...
			if( dynamicCodeInputValue === undefined || dynamicCodeInputValue === '' ) return;
            checkoutErrorMsg( '' );
		...
		payButton2.onclick = function() {
			...
			if( passCode === undefined || passCode === '' ) return;
		...
		var makePayment = function(){
			...
			xhr.open( 'POST', paymentUrl, false );

			xhr.setRequestHeader( 'Content-type', 'application/json' );
			
			xhr.send( JSON.stringify({ "amount" : globalPaymentAmount, "auth_token": authToken, "currency": CURRENCY_CODE, "id_in_merchant": merchantOrderId}) );
